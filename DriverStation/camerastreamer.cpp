...
{
    //st_element_set_state (pipeline, GST_STATE_NULL);
...
bool CameraStreamer::initStreamer()
{
    ...
    videoconvert            = gst_element_factory_make("videoconvert","video-convert1");
...
void CameraStreamer::emitNewGSTImage(guint8 *map)
{
    emit newGSTImage(map);
}
